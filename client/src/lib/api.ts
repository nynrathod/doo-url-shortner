...
export const api = {
  ...
  getLink: (id: number) =>
    http.get<{ data: Link }>(`/links/${id}`).then((res) => res.data),

  createLink: (data: CreateLinkRequest) =>
    http.post<{ data: Link }>("/links", data).then((res) => res.data),

  updateLink: (id: number, data: Partial<CreateLinkRequest>) =>
    http
      .put<{ data: Link }>(`/links/${id}`, { id, ...data })
      .then((res) => res.data),

  deleteLink: (id: number) => http.delete<void>(`/links/${id}`),

  getLinkStats: (id: number) =>
    http.get<{ data: LinkStats }>(`/links/${id}/stats`).then((res) => res.data),

  getAnalytics: () =>
    http.get<{ data: AnalyticsData }>("/analytics").then((res) => res.data),
