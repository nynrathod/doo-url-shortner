...
export default function LinkDetailsPage() {
    ...
    return (
      <DashboardLayout>
        <div className="flex items-center justify-center min-h-[60vh]">
          <Loader simple className="w-6 h-6 text-neutral-400" />
        </div>
      </DashboardLayout>
    );
  }
  ...
  return (
    <DashboardLayout>
      {/* Fixed Header */}
      <div
        className={`p-6 lg:p-8 bg-white/50 backdrop-blur-sm z-10 shrink-0 ${
          isScrolled
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div className="min-w-0 flex-1">
              ...
              <a
                ...
                className="text-sm text-neutral-500 hover:text-neutral-700 flex items-center gap-1 max-w-full group"
                title={stats.DestinationUrl}
              >
                ↳{" "}
          ...
          <div className="bg-white border border-neutral-200 rounded-lg p-5">
            ...
            <p className="text-lg font-medium text-neutral-900">
              {stats.LastAccessedAt
                ? formatRelativeTime(stats.LastAccessedAt)
                : "Never"}
            </p>
          </div>
              ...
              <AreaChart
                data={(() => {
                  ...
                  for (let i = 23; i >= 0; i--) {
                    ...
                    const hourWeight = Math.abs(
                      Math.sin(((24 - i) / 24) * Math.PI),
                    );
                    const value = Math.max(
                      0,
                      Math.round((hourWeight * stats.ClickCount) / 8),
                    );
        ...
        <div className="mt-6 bg-white border border-neutral-200 rounded-lg">
          ...
          <div className="p-5 space-y-4">
            <div className="flex justify-between py-2 border-b border-neutral-100">
              ...
              <span
                className="text-sm font-medium text-neutral-900 text-right truncate max-w-[60%]"
                title={shortUrl}
              >
                {shortUrl}
              </span>
            ...
            <div className="flex justify-between items-start py-2 border-b border-neutral-100 gap-8">
              <span className="text-sm text-neutral-500 shrink-0 mt-0.5">
                Destination URL
              </span>
              <a
                href={stats.DestinationUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="text-sm font-medium text-blue-600 hover:text-blue-700 hover:underline truncate max-w-[60%] block"
                title={stats.DestinationUrl}
              >
                {stats.DestinationUrl}
              </a>
